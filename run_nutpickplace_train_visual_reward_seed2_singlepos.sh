#!/bin/bash
# Train the ReWiND visual (RGB) reward model on the nut seed2 single_pos rollouts.
#
...
#
# Dataset: 640 episodes, 453 success / 187 fail (70.8%) — near-identical balance
# to the multipos twin (465/175, 72.7%). Unlike gear, nut's fixed pose is NOT
# noticeably easier for the policy. Labels ARE comparable across the two sets:
# both were collected under an A_hard_success config (success_threshold -3.5),
# because the single_pos rollout used the purpose-built
# `A_hard_success_single_pos` baseline rather than plain `single_pos`.
#
# Usage:
#   GPU_ID=5 ./run_nutpickplace_train_visual_reward_seed2_singlepos.sh
#   SKIP_PRECOMPUTE=1 EPOCHS=30 ./...     # features already cached
#   DATA_DIR=... CACHE_DIR=... CKPT_DIR=... ./...   # retarget to another dataset
#
# To train on single_pos + multipos together, point both scripts at a shared
# CACHE_DIR (each precomputes its own root), then re-run either one with
# SKIP_PRECOMPUTE=1 — train_visual_taRL.py globs the whole cache dir
# recursively.
#
...
DATA_DIR=${DATA_DIR:-/mnt/scratch/tactile/nutpickplace_curriculum_seed2_paired}
CACHE_DIR=${CACHE_DIR:-/mnt/scratch/tactile/dino_cache/nut_seed2_singlepos}
CKPT_DIR=${CKPT_DIR:-/mnt/lab-tank/tactile/Tactile-Reward/ckpt_visual/nut_seed2_singlepos}
RUN_NAME=${RUN_NAME:-nut_rgb_seed2_singlepos}
