#!/bin/bash
# Curriculum rollout for peg pickplace — best-ckpt only, one trajectory per
# env, BOTH tactile force field AND RGB front camera saved. Runs twice:
#   1. single_pos baseline → deterministic init pose
#   2. A_legacy baseline   → full randomization (multipos)
...
# Use the BEST peg baselineA_legacy snapshot from the r4tddjlv (May 22-24)
# run — ep_2700 with ~86% success. nn/ also has older vhhj6i3c ckpts of the
# same epoch number, so date-filter to grab only r4tddjlv's.
BEST_EP=2700
...
if [ -z "$CKPT_NAME" ]; then
    echo "[fatal] r4tddjlv ep_${BEST_EP} ckpt not found in $CKPTS_DIR"; exit 1
fi
...
for BASELINE in single_pos A_legacy; do
    if [ "$BASELINE" = "A_legacy" ]; then
        SUBDIR="multipos"
    else
        SUBDIR="$BASELINE"
    fi
