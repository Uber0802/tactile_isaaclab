#!/bin/bash
# Paired (tactile + RGB) rollout for the peg seed2 policy, FIXED init pose.
#
...
#
# Output:
#   /mnt/scratch/tactile/pegpickplace_curriculum_seed2_paired/<run>_ep_NNN/
#     ep<N>_env<XXX>.npy          <- tactile (T=299, 40, 25, 3) float16  ~1.8 MB
#     ep<N>_env<XXX>_camera.npy   <- RGB     (T=299, 224, 224, 3) uint8  ~45 MB
#
# The multi-position twin is run_pegpickplace_curriculum_rollout_seed2_paired_multipos.sh.
#
# ---------------------------------------------------------------------------
...
#
# Usage:
#   WITH_SEED3=1 GPU_ID=5 ./run_pegpickplace_curriculum_rollout_seed2_paired.sh
...
BASE_SAVE_DIR=/mnt/scratch/tactile/pegpickplace_curriculum_seed2_paired
BASELINE=single_pos
...
# e.g. `GPU_ID=5 ./run_pegpickplace_curriculum_rollout_seed2_paired.sh`
