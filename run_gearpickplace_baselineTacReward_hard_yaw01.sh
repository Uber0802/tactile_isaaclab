...
#
# Success-triggered annealing: hold scale at 0.175 until the running episode
# success rate (EMA) first crosses 0.1, then fade 0.175 -> 0.0 over 200 PPO
# iters (gear horizon_length=128 -> 200*128=25600 env control steps). Keeps
# tactile bootstrapping until the policy starts solving, then converges it away
# so it finishes on task reward alone. ANNEAL_STEPS=0 disables (constant scale).
