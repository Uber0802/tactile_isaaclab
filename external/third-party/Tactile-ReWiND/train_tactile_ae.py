...
reconstruction loss on the (T, 40, 25, 3) float16 episodes saved by
FORGE_SAVE_TACTILE_FORCE_FIELD (channels = normal, shear_x, shear_y; rows
0-19 = left finger, 20-39 = right finger). The decoder is thrown away at RL
time; the frozen encoder produces the `tactile_embedding` obs key.
...
def _load_episode(path: str) -> np.ndarray | None:
    """Return (T, 40, 25, 3) float16 frames, or None if unreadable."""
    try:
    ...
    if frames is None or getattr(frames, "ndim", 0) != 4 or frames.shape[0] == 0:
        ...
        return None
    return np.asarray(frames, dtype=np.float16)
