"""Train a tactile ReWiND reward model on the Forge NutThread PickPlace task.

Combines multiple `ep*.npy` data dirs (e.g. baselineA + baselineB rollouts),
holds out N success + N fail trajectories for evaluation, and trains on the
remainder. Each ep file is a dict produced by forge_env._flush_tactile_episode:
    {"Task": str, "Tactile": (T, 40, 25, 3) float16, "Success": int 0/1}

Note: the Task field on disk for nutpickplace dumps is currently the peg string
("grasp peg and insert to another hole"); we override via DEFAULT_NUTTHREAD_INSTRUCTIONS.

Usage:
    python scripts/train_nutthread.py \\
        --data_dirs /mnt/tank/tactile/tactile_dataset/nutpickplace_baselineA \\
                    /mnt/tank/tactile/tactile_dataset/nutpickplace_baselineB \\
        --ckpt_dir /mnt/tank/tactile/Tactile-Reward/checkpoints_nutthread_balanced \\
...
DEFAULT_NUTTHREAD_INSTRUCTIONS = [
...
class NutThreadDataset(Dataset):
    """Per-item schema:
    ...
    def __init__(
        ...
        print(f"[NutThreadDataset] train entries={len(entries)} "
...
def main(args):
    ...
    # ---- instructions ----
    instructions = DEFAULT_NUTTHREAD_INSTRUCTIONS
    ...
    for epoch in range(args.epochs):
        ...
        ckpt_path = os.path.join(args.ckpt_dir, f"nutthread_epoch{epoch}.pth")
        torch.save({
            "args": vars(args),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ckpt_dir",
                    default="/mnt/tank/tactile/Tactile-Reward/checkpoints_nutthread_balanced")
    ap.add_argument("--instructions", nargs="*", default=None,
                    help="Override the default 5 nut-thread paraphrases.")
    ...
    ap.add_argument("--run_name", default="nutthread_balanced")
