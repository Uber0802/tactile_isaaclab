...
class FactoryEnv(DirectRLEnv):
    ...
    def _init_tensors(self):
        ...
        self._use_tip_tcp = (
            "left_elastomer_tip_link" in self._robot.body_names and "right_elastomer_tip_link" in self._robot.body_names
        )
        if self._use_tip_tcp:
            self.left_finger_body_idx = self._robot.body_names.index("left_elastomer_tip_link")
            self.right_finger_body_idx = self._robot.body_names.index("right_elastomer_tip_link")
        else:
            self.left_finger_body_idx = self._robot.body_names.index("panda_leftfinger")
            self.right_finger_body_idx = self._robot.body_names.index("panda_rightfinger")

        if not self._use_tip_tcp and "panda_fingertip_centered" in self._robot.body_names:
            self.fingertip_body_idx = self._robot.body_names.index("panda_fingertip_centered")
        else:
            self.fingertip_body_idx = None

        if "force_sensor" in self._robot.body_names:
            self.tcp_orientation_body_idx = self._robot.body_names.index("force_sensor")
        elif self.fingertip_body_idx is not None:
            self.tcp_orientation_body_idx = self.fingertip_body_idx
        else:
            self.tcp_orientation_body_idx = self.left_finger_body_idx
    ...
    def _compute_intermediate_values(self, dt):
        ...
        left_pos = self._robot.data.body_pos_w[:, self.left_finger_body_idx] - self.scene.env_origins
        right_pos = self._robot.data.body_pos_w[:, self.right_finger_body_idx] - self.scene.env_origins
        self.fingertip_midpoint_pos = (left_pos + right_pos) * 0.5

        if self.fingertip_body_idx is not None:
            self.fingertip_midpoint_quat = self._robot.data.body_quat_w[:, self.fingertip_body_idx]
            self.fingertip_midpoint_linvel = self._robot.data.body_lin_vel_w[:, self.fingertip_body_idx]
            self.fingertip_midpoint_angvel = self._robot.data.body_ang_vel_w[:, self.fingertip_body_idx]
        else:
            self.fingertip_midpoint_quat = self._robot.data.body_quat_w[:, self.tcp_orientation_body_idx]
            self.fingertip_midpoint_linvel = (
                self._robot.data.body_lin_vel_w[:, self.tcp_orientation_body_idx]
            )
            self.fingertip_midpoint_angvel = (
                self._robot.data.body_ang_vel_w[:, self.tcp_orientation_body_idx]
            )
    ...
    def set_pos_inverse_kinematics(
        ...
        preferred_joint_pos = torch.tensor(self.cfg.ctrl.reset_joints, device=self.device).unsqueeze(0).repeat(
            len(env_ids), 1
        )
        identity = torch.eye(7, device=self.device).unsqueeze(0).repeat(len(env_ids), 1, 1)
        posture_gain = torch.tensor([0.05, 0.05, 0.08, 0.2, 0.35, 0.35, 0.25], device=self.device).unsqueeze(0)
        max_joint_step = 0.2
        ik_time = 0.0
        while ik_time < 0.25:
            ...
            current_joint_pos = self.joint_pos[env_ids, 0:7]
            posture_error = preferred_joint_pos - current_joint_pos
            posture_error = torch.atan2(torch.sin(posture_error), torch.cos(posture_error))
            nullspace_projector = identity - jacobian_pinv @ jacobian
            weighted_posture_error = posture_gain * posture_error
            posture_delta = (nullspace_projector @ weighted_posture_error.unsqueeze(-1)).squeeze(-1)
            delta_dof_pos[:, 0:7] += posture_delta
            delta_dof_pos[:, 0:7] = torch.clamp(delta_dof_pos[:, 0:7], min=-max_joint_step, max=max_joint_step)
    ...
    def get_handheld_asset_relative_pose(self):
        ...
        if self.cfg_task.name == "peg_insert":
            ...
            fingerpad_length = self.cfg_task.robot_cfg.franka_fingerpad_length
            if getattr(self, "_use_tip_tcp", False):
                # GelSight tips extend past the stock Franka fingerpad geometry.
                fingerpad_length += 0.008
            held_asset_relative_pos[:, 2] -= fingerpad_length
            held_asset_relative_pos += torch.tensor(
                self.cfg_task.held_asset_in_hand_pos_offset, device=self.device
            ).unsqueeze(0)
    ...
    def _set_franka_to_default_pose(self, joints, env_ids):
        ...
        if getattr(self, "_use_tip_tcp", False):
            # Keep a small clearance for the thicker GelSight fingertips without leaving the peg ungrasped.
            gripper_width += 0.0015
    ...
    def randomize_initial_state(self, env_ids):
        ...
        while True:
            ...
            orn_noise_quat = torch_utils.quat_from_euler_xyz(
                roll=above_fixed_orn_noise[:, 0], pitch=above_fixed_orn_noise[:, 1], yaw=above_fixed_orn_noise[:, 2]
            )
            hand_task_quat[bad_envs, :] = torch_utils.quat_mul(default_hand_quat[bad_envs], orn_noise_quat)
            hand_task_pos[bad_envs, :] = above_fixed_pos[bad_envs]
        ...
        # (3) Randomize asset-in-gripper location from the sampled gripper task pose,
        # not from the post-IK measured pose. This keeps peg-in-hand geometry
        # consistent even when IK converges imperfectly for the custom fingertips.
        ...
        grasp_time = 0.0
        while grasp_time < 0.5:
            gripper_close_target = 0.0
            self.ctrl_target_joint_pos[env_ids, 7:] = gripper_close_target
            self.close_gripper_in_place()
            self.step_sim_no_action()
            grasp_time += self.sim.get_physics_dt()

        if os.getenv("FACTORY_CHECK_GRIPPER_CLOSE", "0") == "1":
            check_env_id = min(71, self.num_envs - 1)
            finger_joint_pos = self.joint_pos[:, 7:9].detach().cpu()
            print(
                "[CHECK][gripper_close]",
                {
                    "env": int(check_env_id),
                    "env_finger_joint_pos": finger_joint_pos[check_env_id].tolist(),
                    "mean_finger_joint_pos": finger_joint_pos.mean(dim=0).tolist(),
                    "min_finger_joint_pos": finger_joint_pos.min(dim=0).values.tolist(),
                    "max_finger_joint_pos": finger_joint_pos.max(dim=0).values.tolist(),
                },
            )
