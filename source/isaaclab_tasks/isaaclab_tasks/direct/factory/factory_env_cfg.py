...
LOCAL_PEG_INSERT_ROBOT_USD_PATH = "/mnt/home/uber/IsaacLab/franka_gelsight_only.usd"

PEG_INSERT_ROBOT_USD_PATH = (
    LOCAL_PEG_INSERT_ROBOT_USD_PATH if os.path.exists(LOCAL_PEG_INSERT_ROBOT_USD_PATH) else f"{ASSET_DIR}/franka_mimic.usd"
)
...
class CtrlCfg:
    ...
    reset_joints = [-0.4536, 0.1362, 0.3922, -2.3182, -0.1029, 2.2230, 0.7862]
    reset_task_prop_gains = [300, 300, 300, 20, 20, 20]
    reset_rot_deriv_scale = 10.0
    default_task_prop_gains = [100, 100, 100, 30, 30, 30]

    # Null space parameters.
    default_dof_pos_tensor = [-0.4536, 0.1362, 0.3922, -2.3182, -0.1029, 2.2230, 0.7862]
    kp_null = 10.0
    kd_null = 6.3246
...
class FactoryTaskPegInsertCfg(FactoryEnvCfg):
    ...
    def __post_init__(self):
        super().__post_init__()
        self.robot = self.robot.replace(spawn=self.robot.spawn.replace(usd_path=PEG_INSERT_ROBOT_USD_PATH))
