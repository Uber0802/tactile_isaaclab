...
parser = argparse.ArgumentParser(
    description="Generate High-Fidelity SDF collision USD for the YCB potted meat can."
)

parser.add_argument(
    "--output",
    type=str,
    default="assets/Props/potted_meat_can_sdf.usd",
...
parser.add_argument(
    ...
    type=float,
    default=0.02,
    help="Mass to set on the rigid body in kg. If not specified, defaults to 0.05 scaled by target_scale^3.",
)
...
def _apply_robust_sdf_collision(mesh_prim: Usd.Prim, sdf_resolution: int):
    ...
    # SDF collision parameters — solid object, tight margins for accurate contact geometry
    ...
    # Triangle mesh fallback — used for raycasts and as a CPU-side backup
    # when SDF contact generation misses fast-moving contacts.
    PhysxSchema.PhysxTriangleMeshCollisionAPI.Apply(mesh_prim)

def main():
    # YCB Potted Meat Can 010
    input_path = "https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Isaac/5.1/Isaac/Props/YCB/Axis_Aligned/010_potted_meat_can.usd"
    output_path = os.path.abspath(args.output)
    
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    print(f"Opening remote Potted Meat Can USD: {input_path}")
