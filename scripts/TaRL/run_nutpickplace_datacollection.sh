#!/bin/bash
# Nut pickplace paired data collection — load the best baseline ckpt, roll out
...
[ -z "$CKPT_NAME" ] && { echo "[fatal] ep_${BEST_EP} ckpt not found in $CKPTS_DIR"; exit 1; }
...
for BASELINE in single_pos baseline; do
    ...
    ./isaaclab.sh -p "$TRAIN" \
        ...
        agent.params.config.full_experiment_name=NutThread_PickPlace_datacollection_tmp
done
