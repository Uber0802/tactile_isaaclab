#!/bin/bash
# Paired (tactile + RGB) rollout for gear pickplace — best-ckpt only, one
# trajectory per env, BOTH tactile force field AND RGB front camera saved.
# Runs twice:
...
# Gear obs disables the yaw-diff channel (FORGE_DISABLE_YAW_DIFF_OBS=1) to match
# how the gear baselineA policy was trained.
...
BASE_SAVE_DIR=/mnt/tank/tactile/tactile_dataset/gearpickplace_paired

# Use the BEST gear baselineA snapshot (ep_740, ~79% success — curriculum peak).
BEST_EP=740
CKPT_NAME=$(find "$CKPTS_DIR" -name "last_ForgeGearPickPlace_ep_${BEST_EP}_rew_*.pth" 2>/dev/null | head -1)
if [ -z "$CKPT_NAME" ]; then
