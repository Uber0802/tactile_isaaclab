...
parser = argparse.ArgumentParser(
    description="Generate High-Fidelity SDF collision USD for the YCB mug."
)
...
parser.add_argument(
    "--sdf-resolution",
    type=int,
    default=256, # Increased for thin walls
...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Mass to set on the rigid body in kg. If not specified, defaults to 0.02 scaled by target_scale^3 (physically-accurate volume scaling).",
)
...
def _apply_robust_sdf_collision(mesh_prim: Usd.Prim, sdf_resolution: int):
    ...
    # SDF collision parameters (tuned for thin walls)
    sdf_api = PhysxSchema.PhysxSDFMeshCollisionAPI.Apply(mesh_prim)
    sdf_api.CreateSdfResolutionAttr(sdf_resolution)
    sdf_api.CreateSdfMarginAttr(0.00025 * scale)
    sdf_api.CreateSdfNarrowBandThicknessAttr(0.0005 * scale)
    sdf_api.CreateSdfSubgridResolutionAttr(6)

    # PhysX tuning
    physx_collision = PhysxSchema.PhysxCollisionAPI.Apply(mesh_prim)
    physx_collision.CreateContactOffsetAttr(0.0005 * scale)
    physx_collision.CreateRestOffsetAttr(0.0)
    physx_collision.CreateMinTorsionalPatchRadiusAttr(0.005)

def main():
    # YCB Mug 025
    input_path = "https://omniverse-content-production.s3-us-west-2.amazonaws.com/Assets/Isaac/5.1/Isaac/Props/YCB/Axis_Aligned/024_bowl.usd"
    output_path = os.path.abspath(args.output)
    
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    print(f"Opening remote Mug USD: {input_path}")
